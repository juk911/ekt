...
?>
	...
	<form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
        ...
        <div class="form-group">
            <input type="email" class="footer-subsribe-form form-control" type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>">
		</div>
        <div class="form-group">
            <select class="form-control footer-subsribe-form" id="exampleFormControlSelect1" placeholder="Укажите, кто Вы?">
                <option disabled selected>Укажите, кто Вы?</option>
                <option>Человек</option>
                <option>Животное</option>
                <option>Инопланетянин</option>
            </select>
        </div>
		...
		<?endif;?>
			...
			<button class="footer-subsribe-btn btn btn-primary" id="bx_subscribe_btn_<?=$buttonId?>"><span><?=GetMessage("subscr_form_button")?></span></button>
